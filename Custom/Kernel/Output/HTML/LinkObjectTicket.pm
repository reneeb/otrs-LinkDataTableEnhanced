...
# ---
        my $TitleCss = '';
        if ( $Ticket->{StateType} =~ m{pending}xms && $Ticket->{UntilTime} <= 0 ) {
            $TitleCss = 'PendingTimeWarning';
        }
# ---
...
# ---
#                MaxLength => 50,
                MaxLength => $Self->{ConfigObject}->{'Minati::LinkedTextLength'} || 50,
# ---
            },
            {
                Type    => 'Text',
...
# ---
            {
                Type    => 'Text',
                Content => $Self->{UserObject}->UserName( UserID => $Ticket->{OwnerID} ) || $Ticket->{Owner},
            },
# ---
        );
